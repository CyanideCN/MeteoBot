...
async def ec_r24(fxhour:str):
    init_time = get_latest_run() + datetime.timedelta(hours=8)
    fname = init_time.strftime('%y%m%d%H') + '.{}'.format(fxhour)
    f_string = '{}_{}_EC_R24.png'.format(get_latest_run().strftime('%Y%m%d%H'), fxhour)
    fpath = pic_root.joinpath('mpkit', f_string).as_posix()
    if os.path.exists(fpath):
        return 'mpkit/' + f_string
...
async def ec_ptype(fxhour:str):
    init_time = get_latest_run() + datetime.timedelta(hours=8)
    fname = init_time.strftime('%y%m%d%H') + '.{}'.format(fxhour)
    f_string = '{}_{}_EC_PTYPE.png'.format(get_latest_run().strftime('%Y%m%d%H'), fxhour)
    fpath = pic_root.joinpath('mpkit', f_string).as_posix()
    if os.path.exists(fpath):
        return 'mpkit/' + f_string
...
async def sh_cr(fxhour:str):
    init_time = get_latest_run() + datetime.timedelta(hours=8)
    fname = init_time.strftime('%y%m%d%H') + '.{}'.format(fxhour)
    f_string = '{}_{}_SHHR_CR.png'.format(get_latest_run().strftime('%Y%m%d%H'), fxhour)
    fpath = pic_root.joinpath('mpkit', f_string).as_posix()
    if os.path.exists(fpath):
        return 'mpkit/' + f_string
