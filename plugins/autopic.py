...
from .easygrib import ModelData
...
def get_latest_run(offset=6, delta=12):
    now = datetime.datetime.utcnow()
    if now.hour >= offset and now.hour <= (24 - offset):
        return datetime.datetime(now.year, now.month, now.day, 0)
    elif now.hour < offset:
        return datetime.datetime(now.year, now.month, now.day, 0) - datetime.timedelta(hours=delta)
    else:
        return datetime.datetime(now.year, now.month, now.day, 0) + datetime.timedelta(hours=delta)
...
class process(object):
    def __init__(self, pic_name, prod_path, time_mode='model', delay=0.5):
    ...
    def __call__(self, func):
        async def deco(*args, **kwargs):
            rtime = kwargs.pop('time', None)
            if not rtime:
                if self.tm == 'model':
                    lr = get_latest_run()
                elif self.tm == 'realtime':
                    lr = datetime.datetime.utcnow() - datetime.timedelta(hours=self.delay)
            else:
                lr = datetime.datetime.strptime(str(rtime), '%Y%m%d%H')
            init_time = lr + datetime.timedelta(hours=8)
            if self.tm == 'realtime':
                fname = init_time.strftime('%y%m%d%H.000')
                fxhour = 0
            else:
                fxhour = args[0]
            ...
            if not georange:
                georange = kwargs.pop('gr', None)
                if not georange:
                    georange = (17, 42, 90, 125)
            kwargs['georange'] = georange
            f_string = '{}_{}_{}_'.format(lr.strftime('%Y%m%d%H'), fxhour, self.pic_name)
    ...
    if isinstance(data, list):
        ...
        if data:
            ...
            p.contour(h500, levels=np.arange(4000, 6000, 40), clabeldict={'levels':np.arange(4000, 6000, 40)}, color='black',
                    lw=0.3)
            p.contour(t850, levels=0, color='red', lw=0.3)
            p.drawcoastline()
            p.drawprovinces()
            p.drawparameri(lw=0)
            p.colorbar(c)
...
async def ec_r24(fxhour:str, data=None, **kw):
    if isinstance(data, list):
        ...
        if data:
            ...
            georange = kw.pop('georange')
            x, y = data[0].data['Lon'], data[0].data['Lat']
            ret = clip_data(x, y, data[0].data['Grid'], georange[2], georange[3], georange[0], georange[1])[::-1]
            lon, lat = np.meshgrid(x, y)
            lon = clip_data(x, y, lon, georange[2], georange[3], georange[0], georange[1])[::-1]
            lat = clip_data(x, y, lat, georange[2], georange[3], georange[0], georange[1])[::-1]
            p = Plot()
            ...
            p.xx = lon
            p.yy = lat
            c = p.contourf(ret, gpfcmap='wxb.rain')
            p.drawcoastline()
            p.drawprovinces()
            p.drawparameri(lw=0)
            p.colorbar(c)
...
async def ec_sweat(fxhour:str, data=None, **kw):
    if isinstance(data, list):
        ...
        if data:
            ...
            p.timestamp(lr.strftime('%Y%m%d%H'), int(fxhour))
            p.maxminnote(ret[~np.isnan(ret)], 'SWEAT', '')
            p.save(fpath)
        return 'mpkit/' + f_string
    else:
        return None
...
@process('EC_GUST_P6', ['ECMWF_HR/10_METRE_WIND_GUST_IN_THE_LAST_6_HOURS/'])
async def ec_gust6(fxhour:str, data=None, **kw):
    if isinstance(data, list):
        f_string = kw.pop('fs')
        if data:
            lr = kw.pop('time')
            ...
            p.xx = lon
            p.yy = lat
            gust = clip_data(x, y, data[0].data['Grid'], georange[2], georange[3], georange[0], georange[1])[::-1]
            c = p.contourf(gust, gpfcmap='tt.850w_metric')
            p.drawcoastline()
            p.drawprovinces()
            p.drawparameri(lw=0)
            p.colorbar(c)
            p.title('ECMWF 10m Wind Gust in Past 6 Hours (Generated by QQbot)', nasdaq=False)
            p.maxminnote(gust, 'Wind', 'm/s')
            p.timestamp(lr.strftime('%Y%m%d%H'), int(fxhour))
            p.save(fpath)
        return 'mpkit/' + f_string
    else:
...
@process('EC_LSHEAR', ['ECMWF_HR/UGRD/1000/', 'ECMWF_HR/VGRD/1000/', 'ECMWF_HR/UGRD/850/', 'ECMWF_HR/VGRD/850/'])
async def ec_llshear(fxhour:str, data=None, **kw):
    if isinstance(data, list):
        f_string = kw.pop('fs')
        if data:
            lr = kw.pop('time')
            ...
            p.xx = lon
            p.yy = lat
            shear = np.sqrt((data[0].data['Grid'] - data[2].data['Grid'])**2 + (data[1].data['Grid'] - data[3].data['Grid'])**2)
            s = clip_data(x, y, shear, georange[2], georange[3], georange[0], georange[1])[::-1]
            c = p.contourf(s, gpfcmap='tt.850w_metric')
            p.drawcoastline()
            p.drawprovinces()
            p.drawparameri(lw=0)
            p.colorbar(c)
            p.title('ECMWF 1000hPa - 850hPa Wind Shear (m/s) (Generated by QQbot)', nasdaq=False)
            p.timestamp(lr.strftime('%Y%m%d%H'), int(fxhour))
            p.save(fpath)
        return 'mpkit/' + f_string
    else:
...
@process('EC_APCP', ['ECMWF_HR/APCP/'])
async def ec_rt(fxhour:str, data=None, **kw):
    if isinstance(data, list):
        f_string = kw.pop('fs')
        if data:
            lr = kw.pop('time')
            ...
            p.setmap(georange=georange)
            p.setxy((y.min(), y.max(), x.min(), x.max()), 0.125)
            c = p.contourf(data[0].data['Grid'][::-1], gpfcmap='wxb.raint')
            p.drawcoastline()
            p.drawprovinces()
            p.drawparameri(lw=0)
            p.colorbar(c)
            p.title('ECMWF Accumulated Total Precipitation (mm) (Generated by QQbot)', nasdaq=False)
            p.timestamp(lr.strftime('%Y%m%d%H'), int(fxhour))
            p.save(fpath)
        return 'mpkit/' + f_string
    else:
...
FUNC_CONV = {'T2M':ec_t2m, 'T850H500':ec_t850_h500, 'UV850H500':ec_uv850_h500, 'ASNOW':ec_asnow,
             ...
             'LCL':ec_lcl, 'GUST06':ec_gust6, 'LSHEAR':ec_llshear, 'APCP':ec_rt}
...
@process('SHHR_SWEAT', ['SHANGHAI_HR/UGRD/850/', 'SHANGHAI_HR/VGRD/850/', 'SHANGHAI_HR/UGRD/500/',
                        'SHANGHAI_HR/VGRD/500/', 'SHANGHAI_HR/TMP/850/', 'SHANGHAI_HR/TMP/500/', 'SHANGHAI_HR/RH/850/'])
async def sh_sweat(fxhour:str, data=None, **kw):
    if isinstance(data, list):
        f_string = kw.pop('fs')
        if data:
            lr = kw.pop('time')
            ...
            p.drawparameri(lw=0)
            p.colorbar(c)
            p.title('Shanghai HRES SWEAT Index, 850hPa Streamlines (Generated by QQbot)', nasdaq=False)
            p.timestamp(lr.strftime('%Y%m%d%H'), int(fxhour))
            p.maxminnote(ret[~np.isnan(ret)], 'SWEAT', '')
            p.save(fpath)
        return 'mpkit/' + f_string
...
@process('CLDAS_T2M', ['CLDAS/TMP/2M_ABOVE_GROUND/'], time_mode='realtime')
async def cldas_t2m(data=None, **kw):
    if isinstance(data, list):
        f_string = kw.pop('fs')
        if data:
            lr = kw.pop('time')
            ...
            p.drawparameri(lw=0)
            p.colorbar(c)
            p.title('CLDAS 2m Temperature (Generated by QQbot)', nasdaq=False)
            p._timestamp(lr.strftime('%Y/%m/%d %a %HZ'))
            #p.maxminnote(t2m, 'Temp', '°C')
            p.save(fpath)
        return 'mpkit/' + f_string
    else:
        return None

@process('CLDAS_R01', ['CLDAS/RAIN01/'], time_mode='realtime')
async def cldas_r01(data=None, **kw):
    if isinstance(data, list):
        f_string = kw.pop('fs')
        if data:
            lr = kw.pop('time')
            ...
            p.xx = lon
            p.yy = lat
            prec = clip_data(x, y, data[0].data['Grid'], georange[2], georange[3], georange[0], georange[1])[::-1]
            prec[prec == 9999] = 0
            c = p.contourf(prec, gpfcmap='wxb.rain1')
            p.drawcoastline()
            p.drawprovinces()
            p.drawparameri(lw=0)
            p.colorbar(c)
            p.maxminnote(prec, 'Prec.', 'mm')
            p.title('CLDAS 1-Hour Precipitation (mm) (Generated by QQbot)', nasdaq=False)
            p._timestamp(lr.strftime('%Y/%m/%d %a %HZ'))
            p.save(fpath)
        return 'mpkit/' + f_string
    else:
...
@process('CLDAS_TR01', ['CLDAS/RAIN01_TRI_DATA_SOURCE/'], time_mode='realtime', delay=1)
async def cldas_tr01(data=None, **kw):
    if isinstance(data, list):
        f_string = kw.pop('fs')
        if data:
            lr = kw.pop('time')
            ...
            p.colorbar(c)
            p.maxminnote(prec, 'Prec.', 'mm')
            p.title('CLDAS Tri Data Source 1-Hour Precipitation (mm) (Generated by QQbot)', nasdaq=False)
            p._timestamp(lr.strftime('%Y/%m/%d %a %HZ'))
            p.save(fpath)
        return 'mpkit/' + f_string
    else:
...
FUNC_CONV_CLDAS = {'T2M':cldas_t2m, 'R01':cldas_r01, 'TR01':cldas_tr01}

@on_command('CLDAS', only_to_me=False)
async def call_cldas_func(session:CommandSession):
    PERMITUSERS = get_perm_usr()
    ids = get_id(session)
    if ids not in PERMITUSERS:
        await session.send('此功能为付费功能，请付费后调用')
        raise PermissionError('Permission denied')
    raw = session.ctx['raw_message'].split('CLDAS')[1].strip()
    command = raw.split(' ')
    c0 = command[0]
    c_group = c0.split('-')
    func = FUNC_CONV_CLDAS[c_group[0]]
    if len(command) > 1:
        args = command[1:]
        shell = args_parser(' '.join(args))
    else:
        shell = {}
    db = DBRecord()
    db.cldas(ids, c_group[0])
    fpath = await func(**shell)
    if fpath:
        await session.send('[CQ:image,file={}]'.format(fpath))
    else:
        await session.send('当前预报时效无数据')
...
async def gfs_srh(fxhour:str, **kw):
    now = datetime.datetime.utcnow()
    hour = now.hour
    if hour in range(4, 10):
        init_h = datetime.datetime(now.year, now.month, now.day, 0)
    elif hour in range(11, 16):
        init_h = datetime.datetime(now.year, now.month, now.day, 6)
    elif hour in range(17, 22):
        init_h = datetime.datetime(now.year, now.month, now.day, 12)
    elif hour == 23:
        init_h = datetime.datetime(now.year, now.month, now.day, 18)
    else:
        init_h = datetime.datetime(now.year, now.month, now.day, 18) - datetime.timedelta(days=1)
    init = init_h.strftime('%Y%m%d%H')
    georange = kw.pop('georange', (17, 42, 90, 125))
    f_string = '{}_{}_{}_'.format(init_h.strftime('%Y%m%d%H'), fxhour, 'GFS_SRH')
    f_string += '_'.join([str(i) for i in georange]) + '.png'
    fpath = pic_root.joinpath('mpkit', f_string).as_posix()
    srh = ModelData('GFS', init, int(fxhour), georange, parameter='HLCY', level='3000-0_m_above_ground')
    p = Plot()
    p.setfamily('Lato')
    p.setdpi(200)
    p.setmap(georange=georange)
    p.setxy(georange, 0.25)
    c = p.contourf(srh, gpfcmap='sweat')
    lvl = np.arange(100, 2000, 100)
    p.contour(srh, levels=lvl, clabeldict={'levels':lvl}, color='black', lw=0.4, alpha=0.6)
    p.drawcoastline()
    p.drawprovinces()
    p.drawparameri(lw=0)
    p.colorbar(c)
    p.title('GFS 0-3 km Storm Relative Helicity (J/kg) (Generated by QQbot)', nasdaq=False)
    p.timestamp(init, int(fxhour))
    p.save(fpath)
    return 'mpkit/' + f_string

FUNC_CONV_GFS = {'SRH':gfs_srh}

@on_command('GFS', only_to_me=False)
async def call_gfs_func(session:CommandSession):
    PERMITUSERS = get_perm_usr()
    ids = get_id(session)
    if ids not in PERMITUSERS:
        await session.send('此功能为付费功能，请付费后调用')
        raise PermissionError('Permission denied')
    raw = session.ctx['raw_message'].split('GFS')[1].strip()
    command = raw.split(' ')
    func = FUNC_CONV_GFS[command[0]]
    try:
        fx = int(command[1])
    except Exception as e:
        await session.send(e)
    db = DBRecord()
    db.gfs(ids, command[0], fx)
    fs = convert_time(fx)
    if len(command) > 2:
        args = command[2:]
        shell = args_parser(' '.join(args))
