...
class process(object):
    def __init__(self, pic_name, prod_path, time_mode='model'):
        ...
        async def deco(*args, **kwargs):
            ...
            if not rtime:
                if self.tm == 'model':
                    ...
                    lr = datetime.datetime.utcnow() - datetime.timedelta(hours=1)
            else:
...
async def cldas_t2m(data=None, **kw):
    if isinstance(data, list):
        ...
        if data:
            ...
            p.timestamp(lr.strftime('%Y%m%d%H'), 0)
            p.save(fpath)
        return 'mpkit/' + f_string
    else:
        return None    
