...
_DATADIR_ = r'D:\ModelData'
_T1534STEP_ = 180 / 1536
_GFSSTEP_ = 0.25
_ECMWFSTEP_ = 0.5
_ERASTEP_ = 0.75
...
def GFS(level, parameter, basetime, fcsthour, georange, filename=None, mode='op'):
    ...
    if filename == None:
        if mode == 'op':
            file_s = '%s_%s_%s_%d%d%d%d_%d.grib2'
        elif mode == '1534':
            file_s = '%s_%s_%s_%d%d%d%d_%d_1534.grib2'
        filename = path.join(_DATADIR_, file_s % (basetime, level, parameter, latmin,
                                                 latmax, lonmin, lonmax, fcsthour))
    ...
    if mode == 'op':
        ...
        url_prefix = 'http://nomads.ncep.noaa.gov/cgi-bin/filter_gfs_sflux.pl?'
        if fcsthour < 10:
            f_s = '0' + str(fcsthour)
        else:
            f_s = str(fcsthour)
        url_fcst = 'file=gfs.t%sz.sfluxgrbf%s.grib2' % (basetime[-2:], f_s)
...
def ModelData(model, basetime, fcsthour, georange, **kwargs):
    ...
    **kwargs: level parameter(GFS/ERA), product(ECMWF)'''
    ...
    if latmax < latmin or lonmax < lonmin:
        ...
        return
    if model.upper() in ('G', 'GFS', 'T1534'):
        if model.upper() in 'T1534':
            mode = '1534'
        else:
            mode = 'op'
        outname = decoder(GFS(kwargs['level'], kwargs['parameter'], basetime, fcsthour, (latmin, latmax, lonmin, lonmax), mode=mode))
        res = _GFSSTEP_ if mode == 'op' else _T1534STEP_
        nx, ny = int((lonmax - lonmin) / res) + 1, int((latmax - latmin) / res) + 1
